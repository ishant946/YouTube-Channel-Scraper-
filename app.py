    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            if 'entries' in info and info['entries']:
                # BUG FIX: Get the ID and build the URL, don't look for 'url' key
        ...
        st.error("Could not check languages. The channel might be invalid, have no public videos, or be private. Please check the URL.")
...
def clean_transcript(vtt_content):
    ...
    transcript_lines = []
    for line in lines:
        if not line.strip() or '-->' in line or line.startswith('WEBVTT') or line.startswith('Kind:') or line.startswith('Language:'):
            continue
